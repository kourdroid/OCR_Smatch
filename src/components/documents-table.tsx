...
// Optimization: Memoized component to prevent unnecessary re-renders when other rows change
const GroupRow = memo(function GroupRow({ group, isExpanded, isSelected, onToggle }: GroupRowProps) {
...
// Optimization: Memoized component to prevent unnecessary re-renders when other rows change
const DocumentRowItem = memo(function DocumentRowItem({ document, isSelected, isSubRow = false, onSelect }: DocumentRowItemProps) {
  const ChannelIcon = channelIcons[document.channel as keyof typeof channelIcons] || Mail
  // Unused variable FileIcon removed or used if needed, here it was defined but not used in JSX in original file either?
  // Wait, original code:
  // const FileIcon = fileTypeIcons[document.fileType as keyof typeof fileTypeIcons] || File
  // In JSX: <Badge variant="outline" className="text-xs uppercase">{document.fileType}</Badge>
  // So FileIcon was unused. I'll omit it.
