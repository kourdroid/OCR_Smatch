...
const DocumentRowItem = React.memo(({
  ...
  const FileIcon = fileTypeIcons[document.fileType as keyof typeof fileTypeIcons] || FileText
...
export function DocumentsTable({
  ...
  const toggleGroupExpansion = useCallback((groupId: string) => {
    ...
    // Note: onGroupToggle might not be stable if parent doesn't memoize it,
    // but its effect is minimal here since we are inside a callback.
    if (onGroupToggle) {
              ...
              <TableBody>
                ...
                {paginatedRows.map((row) => {
                  if (row.type === 'group' && row.group) {
                    ...
                    // Calculate isSelected only if selectedDocument is present, to optimize check
                    const isGroupSelected = selectedDocument ? row.group.documents.some((doc) => selectedDocument.id === doc.id) : false
